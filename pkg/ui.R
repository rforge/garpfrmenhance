...
# Define UI for dataset viewer a	pplication
shinyUI(fluidPage(
				...
				fluidRow(h2("Fixed Income Overview")),
				fluidRow(p(a(img(src="cfrm-logo.png", height = 40, width = 304), 
												target="_blank", href="http://depts.washington.edu/compfin/"))),
 	  		fluidRow(p(p(a(img(src="garp_logo_sm.gif", height = 40, width = 304), 
						...
						mainPanel(
								tabsetPanel(
										tabPanel("Discount Factor", 
												wellPanel(verbatimTextOutput("discount.factor")),
												value=1), 
										tabPanel("Bond Pricing",
												wellPanel(verbatimTextOutput("bond.price")),
												wellPanel(plotOutput("bond.price_plot")),               
												value=2),
										tabPanel("Bond Parameters", 
												wellPanel(verbatimTextOutput("bond.parameters")),
												value=3),      
										tabPanel("Data Tab - Spot & Forward Rates and Discount Curve",     
												p(strong("Data Table")),
												wellPanel(dataTableOutput('data.table')),
												p(strong("Result")),
												wellPanel(verbatimTextOutput("spot.rate")),
												p(strong("Plots")),
												wellPanel(plotOutput("spot.rate_plot")),
												value=4),    
										
										tabPanel("About", 
												wellPanel(p(HTML("The application demonstrates functionality available in the GARP-FRM 
